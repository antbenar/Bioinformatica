...
public:
	...
	void desarrollo(){
		...
		for(int i=0; matrizDistancias.size() != 1 ; ++i){
			...
			int pos1 = max(positions.first, positions.second);//nuevo cluster a insertar en el arbol;
			int pos2 = min(positions.first, positions.second);//cluster que ya debe haber sido insertado en el arbol
			
			
			///Se insertan los elementos al Arbol filogenetico
			historial.insert(clusters[pos2],clusters[pos1],distanciaUnida);
