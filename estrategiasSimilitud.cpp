...
private:
	...
	void reducirMatriz( int min_i, int min_j, Operacion& op){
...
public:
	...
	void desarrollo(Operacion& op){
		...
		for(int i=0; matrizDistancias.size() != 2 ; ++i){
			...
			reducirMatriz(positions.first, positions.second, op);	
			cout << "-----------------matriz distancias: " << endl;
			print(matrizDistancias);
		}		
	}
	...
	EstrategiaSimilitud(vector<vector<double>>& matrizDistancias_, string tipoOperacion): matrizDistancias(matrizDistancias_)
	{
		if(tipoOperacion == "min"){
			Min op;
			desarrollo(op);
